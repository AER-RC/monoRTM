...
C     ---  atmospheric optical depths in the microwave region.
...
C-------------------------------------------------------------------------------
      REAL*8 WN(NWN)
      REAL P(NLAY),T(NLAY),W_wv(NLAY),W_o2(NLAY),W_o3(NLAY),
     &     W_n2(NLAY),W_n2O(NLAY),W_co(NLAY),W_so2(NLAY),
     &     W_no2(NLAY),W_oh(NLAY),CLW(NLAY)
      REAL O(NWN,NLAY),OL_WV(NWN,NLAY),
     &     OS_WV(NWN,NLAY),OF_WV(NWN,NLAY),OL_O2(NWN,NLAY),
     &     OL_O3(NWN,NLAY),OL_N2(NWN,NLAY),OC_N2(NWN,NLAY),
     &     OL_N2O(NWN,NLAY),O_CLW(NWN,NLAY),OL_CO(NWN,NLAY),
     &     OL_SO2(NWN,NLAY),OL_NO2(NWN,NLAY),OL_OH(NWN,NLAY)
      LOGICAL INIT
      DATA INIT/.TRUE./
      SAVE INIT
      IF(INIT)THEN
         CALL VECISO            !set-up the isotopes
         CALL READ_HITR(ICP)    !reads the HITRAN data
         INIT=.FALSE.
      ENDIF
     ...
     &      Xn_WV,XN0,XSLF) 
            ...
            OC_N2(M,K)=CONTI_N2(WN(M),T(K),T0,W_n2(K),RFT,RHOFAC,XCN2)!CONT_N2
     ...
     &      OL_OH(M,K)+OS_WV(M,K)+OF_WV(M,K)+OC_N2(M,K)+O_CLW(M,K)
         ENDDO
      ENDDO
      RETURN
      END 
 ...
 10      IF (MOL(I).EQ.MOL_WV)  OL_WV  = OL
   ...
   20    CONTINUE
      ...
      Character Q1*9,Q2*9,HITRAN_FILE*80
      COMMON/HITR/MOL,NBLM,ISO,XNU0,DELTNU,S0,E,ALPS,ALPF,X,XG,NMOLEC
      DATA MOL_WV/1/,MOL_O3/3/,MOL_O2/7/,MOL_N2/22/,MOL_N2O/4/
      DATA MOL_CO/5/,MOL_SO2/9/,MOL_NO2/10/,MOL_OH/13/
      I_WV=0
      ...
      I_OH=0
      NMOLEC=0
      HITRAN_FILE='in/spectral_lines.dat'
      !HITRAN_FILE='tape7_extended'
      !HITRAN_FILE='MODFHITRAN_microw.dat'!(1996) where lcplg modified
      !HITRAN_FILE='tape7_hitran_96_jpl_ext_0_sel_cpl_0_80'!(1996)
      !HITRAN_FILE='MODFHITRAN.dat' !(1996) where lcplg is modified
      !HITRAN_FILE='tape1_monortm_tst_cpl'         !(1986)
      OPEN(9,FILE=HITRAN_FILE,form='formatted',status='old',ERR=20)
 22   READ(9,100,END=3) mo,iso_scal,nu0,S0_scal,R,alpf_scal,
     &     alps_scal,E_scal,X_scal,deltnu_scal,iv1,iv2,
     &     Q1,Q2,ier1,ier2,ier3,iref1,iref2,iref3
      ...
      IF (((iref3.EQ.-1).OR.(iref3.EQ.-3)).AND.(ICP.EQ.1)) THEN !Lines Coupling
         ...
         READ(9,2)J,XNU0(JJ,II),DELTNU(JJ,II),S0(JJ,II),E(JJ,II),
     &        ALPS(JJ,II),ALPF(JJ,II),X(JJ,II),XG(JJ,II)
      ENDIF
      IF (((iref3.EQ.-1).OR.(iref3.EQ.-3)).AND.(ICP.EQ.0)) THEN !Lines Cplng ignored
         READ(9,2)JF,XF,DF,SF,EF,AS,AF,XS,XF
         XG(JJ,II)=0.
      ENDIF
      GOTO 22
 3    CONTINUE
      DO J=1,NMOLEC
         ...
         WRITE(*,'(a40,3i8)') '#of Molecules:',J,MOL(J),NBLM(J)
      ENDDO
      CLOSE(9)
 ...
 2    FORMAT (I2,1P,4(E13.6,E11.4),0P,I2)                           
      RETURN
 20   PRINT *, 'ERROR OPENING HITRAN FILE:',HITRAN_FILE
      STOP
      END   
